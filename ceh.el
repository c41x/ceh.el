...
;; 			      '(("/ce - comment to end of line" . 0)
;; 				("/ca - comment next atom" . 1)
;; 				("/eh - expand header declaration in cpp" . 2)
;; 				("/eha - expand header declarations in cpp (all)" . 3)))
;; 		      :scroll-bar t
;; 		      :height 8
;; 		      :isearch t
;; 		      :isearch-electric t)
;;     (0 (ceh-comment-to-eol))
;;     (1 (ceh-comment-next-atom))
;;     (2 (call-interactively 'ceh-decl-to-impl-namespace))
;;     (3 (call-interactively 'ceh-decl-to-impl-n))))
...
(define-minor-mode ceh-mode
  ...
  :keymap (let ((map (make-sparse-keymap)))
	    ;;(define-key map (kbd "`") 'ceh-command)
